        ...
        <Container>
          <Row>
            <Col style={{textAlign: 'left'}}>
              {data.map ( item => (
                <Fragment key={item.href}><a id="cssFooterMenu" href={item.href} className="footerMenu">{item.title}</a><br/></Fragment>
              ))}
            </Col>
            <Col style={{textDecoration: 'none'}}>
              {socialPages.map ( item => (
                ...
                style={{textDecoration: 'none'}}>
                  <FontIcon variant={"b"} icon={item.icon}/><i/>
                </a>
              ))}
            </Col>
            <Col>
          ...
          <Row>
            <Col>
              <a id="cssCopyright" href="/">© tým 1 #PrevenceMáSmysl {new Date().getFullYear()}</a>
            </Col>
          </Row>
        </Container>
      </div>
